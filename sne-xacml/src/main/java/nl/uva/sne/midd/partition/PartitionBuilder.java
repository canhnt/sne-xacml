...
public class PartitionBuilder {
	...
	/**
	 * Combine two disjoint interval partitions into a single disjoint partition.
	 * Condition: every interval of the result partition belongs to at least an old partition
	 * 
	 * Version 2
	 * 
	 * @param <T>
	...
	public static <T extends Comparable<T>> Partition<T> union(Partition<T> p1, Partition<T> p2) throws MIDDException {
		...
		if (sortedBounds.size() > 0) {
			...
			System.out.println("Empty sortedBound");
		}
 ...
 */
	...
	/**
	 * Return the intersect of two disjoint partition. The returned disjoint partition has items that:
	 * 	- Each interval item is a subset of only an interval I1 \in p1 and a subset of only an interval I2 \in p2.
	 *  
	 * @param p1
	 * @param p2
	 * @return
